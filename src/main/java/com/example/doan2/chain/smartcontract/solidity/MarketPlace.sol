...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    constructor(address _toadKingTokenContract, address _linkWalletAddress) {
        toadKingCoin = IERC20(_toadKingTokenContract);
    ...
    function buyNft(uint256 _marketItemId, int _price, address payable _seller)
        public
        payable
        marketplaceItemExist(_marketItemId)
    {
...
//        uint256 price = nftMarketItem.price;
        uint256 marketFee = (_price * platformFee) / deno;
        ...
        toadKingCoin.transferFrom(msg.sender, address(this), _price);

        toadKingCoin.transfer(_seller, _price - marketFee);

        toadKingCoin.transfer(
            owner(),
            toadKingCoin.balanceOf(address(this))
        );
